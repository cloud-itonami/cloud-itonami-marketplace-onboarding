(ns onboarding.operation
  "OperationActor -- one onboarding request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (SellerOnboardingAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SellerOnboardingGovernor
  ...
  One graph run = one onboarding request (intake -> advise -> govern ->
  ...
  human onboarding/compliance reviewer. The approver resumes with
  `{:approval {:status :approved :by \"..\"}}` (or :rejected).

  Because `:propose-credential` always escalates (governor AND phase,
  independently), the ONLY path by which a seller credential reaches the
  store runs through this interrupt -- a human signature is structurally
  unavoidable, not merely conventional."
...
(defn build
  ...
  opts:
    :advisor      -- an `onboarding.advisor/Advisor` (default: mock-advisor)
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :applicant-id (:applicant-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :applicant-id (:applicant-id request) :by (:by approval)}]}
            {:disposition :hold
